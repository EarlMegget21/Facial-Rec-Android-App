...
import com.backelite.mspr.services.TwitterService;
...
public class AppModule { //ici on déclare toutes nos classes en singleton
    ...
    @Provides
    @Singleton
    public TwitterService provideTwitterService(Retrofit.Builder builder){ //on peut pas le virer d'ici car on n'a pas accès au constructeur de Gson pour lui mettre @Inject (on ne peut pas l'exporter dans un module apart)

        //on le sauvegarde pas car sera utilisé uniquement pour créer le Retrofit permettant de créer le TwitterService qu'on sauvegarde
        OkHttpClient okHttpClient = new OkHttpClient.Builder().build();

        //on le sauvegarde pas car sera utilisé uniquement pour créer le TwitterService qu'on sauvegarde
        Retrofit retrofit = builder.baseUrl("https://api.twitter.com/") //domaine de l'API
                .client(okHttpClient)
                .build();

        return retrofit.create(TwitterService.class); //on créer le service pour l'URL des tweets
    }
